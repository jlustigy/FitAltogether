...
\includegraphics[width=\hsize]{mockdata_quadrature_lc.pdf}
    \end{center}
 \end{minipage}
    \caption{Our mock data based on IGBP classification map of the Earth. }
\label{fig:mockdata}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Principal Component (Hyper-)Plane\\and Graphical Conception}
\label{ss:PCplane}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We show this graphically using the mock datasets shown in Section \ref{s:mockdata}. 

The original data, however, have 4 photometric bands (i.e., $J=4$), and $J$-th dimensional space is inconvenient for visualization. 
We thus reduce the dimensionality using principal component analysis (PCA). 
Unsurprisingly we extract two dominant principal components (PCs) through PCA (other components have vertually zero contribution), which are presented in Figure \ref{fig:PCs}. 

We subtract the 4-band light curves by the time averages of the respective bands, and its trajectory in the PC plane is shown in Figure \ref{fig:trajectory}. 
...
\begin{figure}[t]
    \begin{center}
\includegraphics[width=\hsize]{mockdata_135deg_3types_t12_lc_noreg.png}
    \end{center}
    \caption{Trajectory on the principal component plane.}
...
Conversely, {\it any} set of $\{ {\bf s}_k \}$ that enclose the data points $\{{\bf d}_i\}$ in the hyperplane can be a solution of Equation (\ref{eq:tilde_d_f_ast_s}) subjected to the conditions (\ref{eq:tilde_f_range}) and (\ref{eq:tilde_f_sum}), and the associated matrix, $\fast _{ik}$, can always be found. 
