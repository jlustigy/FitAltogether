...
In this case, the inverse problem described by equation (\ref{eq:Wf}), $f^{\ast }=Wf$, is an overdetermined system, and one must to find the solution for $f$ by some means (e.g. the least squares method), and the resultant $f$ may not automatically satisfy the condition listed by eqution (\ref{eq:cond_f}). 
...
As seen above, the solution is highly degenerate, and in general we cannot reduce to a unique solution. 
...
On the contrary, the spectrally separated surface types correspond to the small time/longitude variations in the apparent area fractions. 
...
The assumption of the smoothness and the small variation of covering area fraction may be imposed using the concept of Gaussian Process \citep{Rasmussen2005}. 
...
\includegraphics[width=\hsize]{xmed_std_regtime_s1e-4.pdf}
    \end{center}
    \caption{Regularization in terms of time. Upper: $\sigma = 0.01$. Lower: $\sigma = 0.0001$}
\label{fig:shrinkwrap}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\includegraphics[width=\hsize]{xmed_std_reglong_s1e-4.pdf}
    \end{center}
    \caption{Regularization in terms of longitude. Upper: $\sigma = 0.01$. Lower: $\sigma = 0.0001$}
