...
For the moment, let us suppose that the planetary radius and the observational geometry are completely known: latitude/longitude of the sub-stellar/sub-observer points, and the distance between the star and the planet (This will be relaxed in \S\ref{s:discussion}.) 
In this case, we can straightforwardly translate the observed planetary intensity to apparent albedo. 
{\it Apparent albedo} is the planetary intensity normalized by that of a loss-less Lambert sphere with the same radius and at the same phase \citep{Qiu2003, Seager2010}; in this paper we will use apparent albedo unless otherwise noted. 

The observed apparent albedo, $d_{ij}$ (``$d$'' for data), can be written using the zenith angle of the insolation, $\theta _0$, and the zenith angle of the observer $\theta _1$, (both defined at each surface point) as follows \citep[see][]{Fujii2010}: 
%%%
\begin{eqnarray}
...
&=& \sum _{i,k} \fast_{ik} \, s_{kj} \label{eq:d_f_ast_s}
\end{eqnarray}
%%%
where
%%%
...
\begin{cases}
\;\; 0 \leq s_{kj} \leq 1 \;\;\; & \mbox{for any $k$, $j$} \\
\;\; 0 \leq f_{lk} \leq 1 \;\;\; & \mbox{for any $l$, $k$} \label{eq:cond_f} \\
...
\begin{figure*}[b]
    ...
    \begin{center}
\includegraphics[width=\hsize]{simpleIGBP_quadrature_lc_regf_PCplane_l90deg_2013.pdf}
    \end{center}
    \end{minipage}
    \begin{minipage}{0.33\hsize}
    \begin{center}
\includegraphics[width=\hsize]{simpleIGBP_quadrature_lc_regf_PCplane_l90deg_2017.pdf}
    \end{center}
    \end{minipage}
    \begin{minipage}{0.33\hsize}
    \begin{center}
\includegraphics[width=\hsize]{simpleIGBP_quadrature_lc_regf_PCplane_l90deg_2019.pdf}
    \end{center}
    \end{minipage}    
    \caption{The vertices of each triangle indicates the sample combination of the colors projected onto the principal component plane, and the colors is scaled by $\mathcal{\tilde F}^{\ast }$ with $\Delta_t=1.75$.The difference between the three panels is just the seed for \url{np.randam.uniform}}
\label{fig:regterm}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{figure*}[b]
    \begin{minipage}{0.33\hsize}
    \begin{center}
\includegraphics[width=\hsize]{simpleIGBP_quadrature_lc_regf_PCplane_l120deg_2013.pdf}
    \end{center}
    \end{minipage}
    \begin{minipage}{0.33\hsize}
    \begin{center}
\includegraphics[width=\hsize]{simpleIGBP_quadrature_lc_regf_PCplane_l120deg_2017.pdf}
    \end{center}
    \end{minipage}
    \begin{minipage}{0.33\hsize}
    \begin{center}
\includegraphics[width=\hsize]{simpleIGBP_quadrature_lc_regf_PCplane_l120deg_2019.pdf}
    \end{center}
    \end{minipage}
    \caption{The vertices of each triangle indicates the sample combination of the colors projected onto the principal component plane, and the colors is scaled by $\mathcal{\tilde F}^{\ast }$ with $\Delta_t\sim 2.3$. The difference between the three panels is just the seed for \url{np.randam.uniform}}
\label{fig:regterm}
\end{figure*}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
