...
By definition, the area fraction $f$ and reflectance spectra $s$ are subject to the following conditions:
%%%
\begin{eqnarray}
\begin{cases}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Fit data with $f$ or with $f^{\ast}$?}
\label{ss:degeneracy}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\memoYF{I am not 100\% sure if the following statement is right. Need to check.}
 
We argue that estimating $\{ f, s \}$ is no superior to estimating $\{ f^{\ast }, s \}$. 
%
One reason comes from the fact the approximation adopted in equation (\ref{eq:discretize}) is valid only when $f_k (\vec \Omega)$ does not change significantly in the $l$-th slice. 
Otherwise, fitting data with $f$ by equation (\ref{eq:Wf}) includes some errors by design. 
...
%
If $L=I$, in general there is a one-to-one relationship between $f^{\ast}_{ik}$ and $f_{lk}$ through $W_{il}$ as far as ${\rm rank}(W)=L(=I)$. 
...
%
If $L<I$, $W$ works as a low-pass filter, as discussed in \citet{Cowan2013}. 
In this case, the inverse problem described by equation (\ref{eq:Wf}), $f^{\ast }=Wf$ is an overdetermined system, and one must to find the solution for $f$ by some means (e.g. least squares method), and the resultant $f$ may not automatically satisfy the condition listed by eqution (\ref{eq:cond_f}). 
However, this is because of the design of the problem and the assumed lowered resolution, and not because of the wrong estimation for $f$ (or $s$). 

Thus, although both $\{ f^{\ast }, s \}$ and $\{ f, s \}$ could be a fitting parameters to the light curves, the estimation for $s$ should not be affected by this conversion in fitting parameters. 
In that sense, $\{ f, s \}$ is not likely to return a better solution than $\{ f^{\ast }, s \}$ does. 



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Strong Degeneracy}
\label{ss:degeneracy}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
